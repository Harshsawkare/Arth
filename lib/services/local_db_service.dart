...
abstract class LocalDBService {
  static const String _boxName = Constants.articlesDbName;
  ...
  static Box<SearchResult> get articlesDb {
  ...
  static Future<void> addSearchResult(
      String keyword, List<Article> articles) async {
    final limitedArticles =
        articles.take(Constants.maxSearchArticleLimit).toList();

    if (!articlesDb.containsKey(keyword) &&
        articlesDb.length >= Constants.maxSearchKeyLimit) {
      // Sort all items by timestamp ascending
      final sorted = articlesDb.toMap().entries.toList()
        ..sort((a, b) => a.value.timestamp.compareTo(b.value.timestamp));

      final oldestKey = sorted.first.key;
      await articlesDb.delete(oldestKey);
    }
    ...
    await articlesDb.put(keyword, result);
  }
  ...
  static List<Article> getArticlesForKeyword(String keyword) {
    final result = articlesDb.get(keyword);
  ...
  static Future<void> deleteKeyword(String keyword) async {
    await articlesDb.delete(keyword);
  }
  ...
  static List<String> getSavedKeywords() {
    final sortedResults = articlesDb.values.toList()
  ...
  static List<SearchResult> getAllSearchResults() => articlesDb.values.toList();
}
